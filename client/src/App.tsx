...
function AdminLayout({ children }: { children: React.ReactNode }) {
 ...
 if (!isAuthenticated || user?.email?.toLowerCase() !== "aki.sokpah.link@gmail.com") return <div className="admin-auth-screen"><div className="admin-auth-card"><img src={logo} alt="A.Tonic"/><span className="eyebrow">Private workspace</span><h1>A.Tonic Studio</h1><p>{isAuthenticated ? "This Google account is not authorized for the studio." : "Sign in with Google to manage music, artwork, licensing and release metadata."}</p>{isAuthenticated ? <button className="button button-dark" onClick={() => logout()}><LogIn size={16}/> Sign out</button> : <button className="button button-dark" onClick={() => startLogin()}><LogIn size={16}/> Continue with Google</button>}{!isLoginConfigured && <small className="auth-config-warning">OAuth is not configured in this deployment. Add VITE_OAUTH_PORTAL_URL and VITE_APP_ID in Vercel.</small>}<Link href="/" className="text-link">Back to website</Link></div></div>;
